...
<style>
body { font-family:'Segoe UI',sans-serif; background:#fff3cd; padding:20px; } /* bg-warning-subtle */
h1 { text-align:center; margin:20px 0; color:#333; }

/* Top back button */
#backHome { position:absolute; top:20px; left:20px; }

/* Submit Button */
#openModal { display:block; margin:0 auto 20px auto; }

/* Recipes Grid */
.recipes { max-width:1200px; margin:30px auto; display:grid; grid-template-columns: repeat(auto-fill,minmax(300px,1fr)); gap:20px; }
.recipe-card { background:#fff; padding:20px; border-radius:15px; box-shadow:0 4px 15px rgba(0,0,0,0.1); transition:0.3s; }
.recipe-card:hover { transform:translateY(-5px); box-shadow:0 8px 20px rgba(0,0,0,0.15); }
.recipe-card img.cuisine-img { width:100%; max-height:200px; object-fit:cover; border-radius:10px; margin-bottom:10px; }
.recipe-card h3 { margin-top:0; color:#222; }
...
.modal-content { background:#fff; margin:50px auto; padding:20px; border-radius:12px; max-width:600px; position:relative; box-shadow:0 4px 15px rgba(0,0,0,0.2);}
#closeModal { position:absolute; top:10px; right:15px; cursor:pointer; font-size:1.5em; }

/* Recipe Form in Modal */
.recipe-form input, .recipe-form textarea, .recipe-form select, .recipe-form button { width:100%; padding:10px; margin-bottom:12px; border-radius:8px; border:1px solid #ccc; font-size:1em; }
.recipe-form button { background:#28a745; color:#fff; border:none; cursor:pointer; transition:0.3s; }
.recipe-form button:hover { background:#218838; }
#cuisineImagePreview { display:none; max-width:100%; max-height:200px; border-radius:8px; margin-top:8px; }

/* Loading overlay */
#loading { display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:9999; text-align:center; color:#fff; font-size:1.5em; padding-top:200px; }
...
<?php if($result && $result->num_rows>0): ?>
    ...
    <div class="recipe-card">
        ...
        <img src="<?= $row['cuisine_image'] ?>" class="cuisine-img">
        <?php endif; ?>
        <h3><?= htmlspecialchars($row['title']) ?></h3>
        <p><?= nl2br(htmlspecialchars($row['description'])) ?></p>
        <p class="meta"><strong>Cuisine:</strong> <?= htmlspecialchars($row['cuisine']) ?> |
           <strong>Difficulty:</strong> <?= $row['difficulty_level'] ?> |
           <strong>By:</strong> <?= htmlspecialchars($row['first_name'].' '.$row['last_name']) ?></p>
        ...
        <div class="comments mt-2">
            <form class="comment-form" data-recipe="<?= $row['recipe_id'] ?>" action="comment_handler.php" method="POST">
